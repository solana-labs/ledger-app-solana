...
void test_parse_system_transfer_message() {
   ...
   SystemTransferInfo system_transfer_info;
   assert(parse_system_transfer_message(&parser, &fee_payer_pubkey, &system_transfer_info) == 0);
   assert(parser.buffer_length == 0);
   assert(system_transfer_info.lamports == 42);
   assert(memcmp(fee_payer_pubkey, system_transfer_info.from, PUBKEY_SIZE) == 0);
}


int main() {
    test_parse_system_transfer_message();
